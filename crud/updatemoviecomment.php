        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatemoviecomment.php">Update Movie Comment</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Movie Comments</h2>
            ...
            // Prepare the query
            $stmt = "SELECT MOVIE_COMMENT.MOVIE_ID,MOVIE.MOVIE_TITLE,MOVIE_COMMENT.MEMBER_ID,MEMBER.MEMBER_NAME,MOVIE_COMMENT.MOVIE_COMMENT
					FROM MOVIE_COMMENT,MOVIE,MEMBER WHERE (MOVIE_COMMENT.MOVIE_ID=MOVIE.MOVIE_ID) AND (MOVIE_COMMENT.MEMBER_ID=MEMBER.MEMBER_ID)
					ORDER BY MOVIE_COMMENT.MOVIE_ID ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Movie</th> <th>Member id</th> <th>Member</th> <th>Comment</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatemoviecomment.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputMovieID']) && !empty($_POST['inputMoviecomment']) ){
                $id = ($_POST['inputMovieID']);
				$moviecomment = ($_POST['inputMoviecomment']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_MOVIE_COMMENT ('$id','$memberID','$moviecomment')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Movie comment $moviecomment with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $moviecomment is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Movie Comment</h2>	
			  
			  <label for="inputMovieID" class="sr-only">Movieid</label>
              <input type="id" name="inputMovieID" id="inputMovieID" class="form-control" placeholder="Enter the ID you want to update" required autofocus>
              <br>
              <label for="inputMoviecomment" class="sr-only">Comment</label>
              <input type="moviecomment" name="inputMoviecomment" id="inputMoviecomment" class="form-control" placeholder="new comment" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update genre</button>
