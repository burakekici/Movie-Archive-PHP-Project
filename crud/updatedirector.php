        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatedirector.php">Update Director</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Directors</h2>
            ...
            // Prepare the query
            $stmt = "SELECT * FROM DIRECTOR ORDER BY DIRECTOR_ID";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Name</th> <th>Surname</th>\n";
              echo "<th>Age</th> <th>Hometown</th>  </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatedirector.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update director
              if (!empty($_POST['inputDirectorname']) && !empty($_POST['inputDirectorsurname']) && !empty($_POST['inputAge']) && !empty($_POST['inputHometown']) ){
                $id = ($_POST['inputDirectorId']);
				$directorname = ($_POST['inputDirectorname']);
                $directorsurname = ($_POST['inputDirectorsurname']);
                $age = ($_POST['inputAge']);
				$hometown = ($_POST['inputHometown']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_DIRECTOR ('$id','$directorname','$directorsurname','$age','$hometown')";
                $stid = oci_parse($conn, $stmt);
                
                // execute the query
                if( !oci_execute($stid) ) {
                  $e = oci_error();
                  echo "<div class=\"alert alert-danger\"><strong>Failed!</strong> Please check the labels.</div>";	
                  echo htmlentities($e['message'], ENT_QUOTES);
                  oci_close($conn);
                } else {
					$currentTime = date("G:i:s");
					$currentDate = date("d-m-Y");
					$stmt = "CALL ADD_LOG('The Director with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $directorname is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Director</h2>	
			  
			  <label for="inputDirectorId" class="sr-only">Directorname</label>
              <input type="id" name="inputDirectorId" id="inputDirectorId" class="form-control" placeholder="Enter the ID you want to update" required autofocus>
              <br>
              <label for="inputDirectorname" class="sr-only">Directorname</label>
              <input type="name" name="inputDirectorname" id="inputDirectorname" class="form-control" placeholder="Directorname" required autofocus>
              <br>
              <label for="inputDirectorsurname" class="sr-only">Directorsurname</label>
              <input type="surname" name="inputDirectorsurname" id="inputDirectorsurname" class="form-control" placeholder="Directorsurname" required autofocus>
              <br>
			  <label for="inputAge" class="sr-only">Age</label>
              <input type="age" name="inputAge" id="inputAge" class="form-control" placeholder="Age" required autofocus>
              <br>
              <label for="inputHometown" class="sr-only">Hometown</label>
              <input type="hometown" name="inputHometown" id="inputHometown" class="form-control" placeholder="Hometown" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update director</button>
