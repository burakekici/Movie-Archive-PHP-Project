        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatedirectorrating.php">Update Director Rating</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Director Rating</h2>
            ...
            // Prepare the query
            $stmt = "SELECT dr.DIRECTOR_ID,d.DIRECTOR_NAME,d.DIRECTOR_SURNAME,dr.MEMBER_ID,m.MEMBER_NAME,dr.DIRECTOR_RATING
					FROM DIRECTOR_RATING dr,DIRECTOR d,MEMBER m WHERE (dr.DIRECTOR_ID=d.DIRECTOR_ID) AND (dr.MEMBER_ID=m.MEMBER_ID) 
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>	Director</th> <th></th> <th>Member id</th> <th>Member</th> <th>Rate</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatedirectorrating.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputDirectorID']) && !empty($_POST['inputDirectorrating']) ){
                $id = ($_POST['inputDirectorID']);
				$directorrating = ($_POST['inputDirectorrating']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_DIRECTOR_RATING ('$id','$memberID','$directorrating')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Director rate $directorrating with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $directorrating is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Director Rate</h2>	
              ...
              <br>
              <label for="inputDirectorrating" class="sr-only">Rate</label>
              <input type="directorrating" name="inputDirectorrating" id="inputDirectorrating" class="form-control" placeholder="new rate" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update Director Rate</button>
