        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updateadmin.php">Update Admin</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Admins</h2>
            ...
            // Prepare the query
            $stmt = "SELECT * FROM MEMBER WHERE MEMBER_TYPE = 1 ORDER BY MEMBER_ID";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Email</th> <th>Password</th>\n";
              echo "<th>Username</th> <th>Age</th> <th>Type</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updateadmin.php">
              ...
              // Update admin
              if (!empty($_POST['inputUsername']) && !empty($_POST['inputUsername']) && !empty($_POST['inputEmail']) && !empty($_POST['inputPassword']) && !empty($_POST['inputAge'])){
                $id = ($_POST['inputId']);
                ...
                // prepare the query
                $stmt = "CALL UPDATE_MEMBER ('$id', '$email','$pass','$username','$age', '1')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The admin with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $username is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              ...
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update admin</button>
            </form>
          </div>
        </div>
      </div>
