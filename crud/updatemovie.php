        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatemovie.php">Update Movie</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Movies</h2>
            ...
            // Prepare the query
            $stmt = "SELECT * FROM MOVIE ORDER BY MOVIE_ID";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Movie</th> <th>Release Date</th> <th>Length</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatemovie.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputMovietitle']) && !empty($_POST['inputReleasedate']) && !empty($_POST['inputLength']) ){
                $id = ($_POST['inputMovieId']);
				$movietitle = ($_POST['inputMovietitle']);
                $releasedate = ($_POST['inputReleasedate']);
                $length = ($_POST['inputLength']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_MOVIE ('$id','$movietitle','$releasedate','$length')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Movie $movietitle with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $movietitle is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Movie</h2>	
			  
			  <label for="inputMovieId" class="sr-only">Movietitle</label>
              ...
              <br>
              <label for="inputMovietitle" class="sr-only">Movietitle</label>
              <input type="movietitle" name="inputMovietitle" id="inputMovietitle" class="form-control" placeholder="Movietitle" required autofocus>
              <br>
              <label for="inputReleasedate" class="sr-only">Releasedate</label>
              <input type="releasedate" name="inputReleasedate" id="inputReleasedate" class="form-control" placeholder="Releasedate" required autofocus>
              <br>
			  <label for="inputLength" class="sr-only">Length</label>
              <input type="length" name="inputLength" id="inputLength" class="form-control" placeholder="Length" required autofocus>
              <br>
