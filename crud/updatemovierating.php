        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatemovierating.php">Update Movie Rating</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Movie Rating</h2>
            ...
            // Prepare the query
            $stmt = "SELECT MOVIE_RATING.MOVIE_ID,MOVIE.MOVIE_TITLE,MOVIE_RATING.MEMBER_ID,MEMBER.MEMBER_NAME,MOVIE_RATING.MOVIE_RATING
					FROM MOVIE_RATING,MOVIE,MEMBER WHERE (MOVIE_RATING.MOVIE_ID=MOVIE.MOVIE_ID) AND (MOVIE_RATING.MEMBER_ID=MEMBER.MEMBER_ID) 
					ORDER BY MOVIE_RATING.MOVIE_ID ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Movie</th> <th>Member id</th> <th>Member</th> <th>Rate</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatemovierating.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputMovieID']) && !empty($_POST['inputMovierating']) ){
                $id = ($_POST['inputMovieID']);
				$movierating = ($_POST['inputMovierating']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_MOVIE_RATING ('$id','$memberID','$movierating')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Movie rate $movierating with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $movierating is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Movie Rate</h2>	
              ...
              <br>
              <label for="inputMovierating" class="sr-only">Rate</label>
              <input type="movierating" name="inputMovierating" id="inputMovierating" class="form-control" placeholder="new rate" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update Movie Rate</button>
