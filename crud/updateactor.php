        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updateactor.php">Update Actor</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Actors</h2>
            ...
            // Prepare the query
            $stmt = "SELECT * FROM ACTOR ORDER BY ACTOR_ID";
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updateactor.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update actor
              if (!empty($_POST['inputActorname']) && !empty($_POST['inputActorsurname']) && !empty($_POST['inputAge']) && !empty($_POST['inputHometown']) ){
                $id = ($_POST['inputActorId']);
				$actorname = ($_POST['inputActorname']);
                $actorsurname = ($_POST['inputActorsurname']);
                $age = ($_POST['inputAge']);
				$hometown = ($_POST['inputHometown']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_ACTOR ('$id','$actorname','$actorsurname','$age','$hometown')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Actor with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $actorname is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Actor</h2>	
			  
			  <label for="inputActorId" class="sr-only">Actorname</label>
              <input type="id" name="inputActorId" id="inputActorId" class="form-control" placeholder="Enter the ID you want to update" required autofocus>
              <br>
              <label for="inputActorname" class="sr-only">Actorname</label>
              <input type="name" name="inputActorname" id="inputActorname" class="form-control" placeholder="Actorname" required autofocus>
              <br>
              <label for="inputActorsurname" class="sr-only">Actorsurname</label>
              <input type="surname" name="inputActorsurname" id="inputActorsurname" class="form-control" placeholder="Actorsurname" required autofocus>
              <br>
              ...
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update actor</button>
