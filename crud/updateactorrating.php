        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updateactorrating.php">Update Actor Rating</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Actor Rating</h2>
            ...
            // Prepare the query
            $stmt = "SELECT ACTOR_RATING.ACTOR_ID,ACTOR.ACTOR_NAME,ACTOR.ACTOR_SURNAME,ACTOR_RATING.MEMBER_ID,MEMBER.MEMBER_NAME,ACTOR_RATING.ACTOR_RATING
					FROM ACTOR_RATING,ACTOR,MEMBER WHERE (ACTOR_RATING.ACTOR_ID=ACTOR.ACTOR_ID) AND (ACTOR_RATING.MEMBER_ID=MEMBER.MEMBER_ID) 
					ORDER BY ACTOR_RATING.ACTOR_ID ";
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updateactorrating.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputActorID']) && !empty($_POST['inputActorrating']) ){
                $id = ($_POST['inputActorID']);
				$actorrating = ($_POST['inputActorrating']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_ACTOR_RATING ('$id','$memberID','$actorrating')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Actor rate $actorrating with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $actorrating is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Actor Rating</h2>	
              ...
              <br>
              <label for="inputActorrating" class="sr-only">Rate</label>
              <input type="actorrating" name="inputActorrating" id="inputActorrating" class="form-control" placeholder="new rate" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update Actor Rating</button>
