        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="deletestoryline.php">Delete Storyline</a></li>
              <br>
              <!-- 6 age_limit -->
		  ...
		  <br><br><br>
            ...
            <form class="form-signin"  method="POST" action = "deletestoryline.php">
              <h2 class="form-signin-heading">Delete Storyline</h2>
              <label for="inputStorylineID" class="sr-only">Movie ID</label>
              <input type="number" name="inputStorylineID" id="inputStorylineID" class="form-control" placeholder="Movie ID" required autofocus>
              <br>
			  ...
			  // Delete movie
              if (!empty($_POST['inputStorylineID'])){
                $id =($_POST['inputStorylineID']);
                
                // prepare the query
                $stmt = "CALL DELETE_STORYLINE ('$id')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('Storyline with ID $id is deleted at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> Storyline is deleted.</div>";
                }
              }
              ob_end_flush();
              ?>
