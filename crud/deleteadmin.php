        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="deleteadmin.php">Delete Admin</a></li>
              <br>
              <!-- 2 user -->
		  ...
		  <br><br><br>
            ...
            <form class="form-signin"  method="POST" action = "deleteadmin.php">
              <h2 class="form-signin-heading">Delete Admin</h2>
              <label for="inputMemberId" class="sr-only">Admin ID</label>
              <input type="number" name="inputMemberId" id="inputMemberId" class="form-control" placeholder="Admin ID" required autofocus>
              <br>
			  ...
			  // Delete admin
              if (!empty($_POST['inputMemberId'])){
                $id = ($_POST['inputMemberId']);
                
                // prepare the query
                $stmt = "CALL DELETE_MEMBER ('$id')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('An admin with ID $id is deleted at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> Admin is deleted.</div>";
                }
              }
              ob_end_flush();
              ?>
