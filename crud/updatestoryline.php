        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatestoryline.php">Update Storyline</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Stroylines</h2>
            ...
            // Prepare the query
            $stmt = "SELECT s.STORYLINE_ID,m.MOVIE_TITLE,s.STORYLINE_CONTEXT FROM STORYLINE s ,MOVIE m WHERE m.MOVIE_ID=s.STORYLINE_ID ORDER BY m.MOVIE_ID";
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatestoryline.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputStorylineID']) && !empty($_POST['inputStorylinecontext']) ){
                $id = ($_POST['inputStorylineID']);
				$storylinecontext = ($_POST['inputStorylinecontext']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_STORYLINE ('$id','$storylinecontext')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Movie storytline $storylinecontext with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $storylinecontext is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Storyline</h2>	
			  
			  <label for="inputStorylineID" class="sr-only">Movieid</label>
              <input type="id" name="inputStorylineID" id="inputStorylineID" class="form-control" placeholder="Enter the ID you want to update" required autofocus>
              <br>
              <label for="inputStorylinecontext" class="sr-only">Storyline</label>
              <input type="storylinecontext" name="inputStorylinecontext" id="inputStorylinecontext" class="form-control" placeholder="new storyline context" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update storyline</button>
