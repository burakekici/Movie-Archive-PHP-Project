        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updateactorcomment.php">Update Actor Comment</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Actor Comments</h2>
            ...
            // Prepare the query
            $stmt = "SELECT ACTOR_COMMENT.ACTOR_ID,ACTOR.ACTOR_NAME,ACTOR.ACTOR_SURNAME,ACTOR_COMMENT.MEMBER_ID,MEMBER.MEMBER_NAME,ACTOR_COMMENT.ACTOR_COMMENT
					FROM ACTOR_COMMENT,ACTOR,MEMBER WHERE (ACTOR_COMMENT.ACTOR_ID=ACTOR.ACTOR_ID) AND (ACTOR_COMMENT.MEMBER_ID=MEMBER.MEMBER_ID) 
					ORDER BY ACTOR_COMMENT.ACTOR_ID ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Actor</th> <th></th> <th>Member id</th> <th>Member</th> <th>Comment</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updateactorcomment.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputActorID']) && !empty($_POST['inputActorcomment']) ){
                $id = ($_POST['inputActorID']);
				$actorcomment = ($_POST['inputActorcomment']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_ACTOR_COMMENT ('$id','$memberID','$actorcomment')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Actor comment $actorcomment with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $actorcomment is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Actor Comment</h2>	
			  
			  <label for="inputActorID" class="sr-only">Actorid</label>
              ...
              <br>
              <label for="inputActorcomment" class="sr-only">Comment</label>
              <input type="actorcomment" name="inputActorcomment" id="inputActorcomment" class="form-control" placeholder="new comment" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update Actor Comment</button>
