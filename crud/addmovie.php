      ...
      <div class="row">
        <div class="col-md-9">
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-9 col-md-offset-3 main">
            <form class="form-signin" method="POST" action = "addmovie.php">
              ...
              if (!empty($_POST['inputMovietitle']) && !empty($_POST['inputReleasedate']) && !empty($_POST['inputLength']) ){
                ...
                // prepare the query
                $stmt = "CALL ADD_MOVIE ('$movietitle','$releasedate','$length')";
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('A new movie with movie title \"$movietitle\" is added at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);

                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $movietitle is added as an movie.</div>";
                }
              ...
              <h2 class="form-signin-heading">Add Movie</h2>

              <label for="inputMovietitle" class="sr-only">Movietitle</label>
              <input type="title" name="inputMovietitle" id="inputMovietitle" class="form-control" placeholder="Movie title" required autofocus>
              <br>
              <label for="inputReleasedate" class="sr-only">Releasedate</label>
              <input type="releasedate" name="inputReleasedate" id="inputReleasedate" class="form-control" placeholder="Release date" required autofocus>
              <br>
