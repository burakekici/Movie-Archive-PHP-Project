        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updatedirectorcomment.php">Update Director Comment</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Director Comments</h2>
            ...
            // Prepare the query
            $stmt = "SELECT dc.DIRECTOR_ID,d.DIRECTOR_NAME,d.DIRECTOR_SURNAME,dc.MEMBER_ID,m.MEMBER_NAME,dc.DIRECTOR_COMMENT
					FROM DIRECTOR_COMMENT dc,DIRECTOR d,MEMBER m WHERE (dc.DIRECTOR_ID=d.DIRECTOR_ID) AND (dc.MEMBER_ID=m.MEMBER_ID) 
					ORDER BY dc.DIRECTOR_ID ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Director</th> <th></th><th>Member id</th> <th>Member</th> <th>Comment</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updatedirectorcomment.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputDirectorID']) && !empty($_POST['inputDirectorcomment']) ){
                $id = ($_POST['inputDirectorID']);
				$directorcomment = ($_POST['inputDirectorcomment']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_DIRECTOR_COMMENT ('$id','$memberID','$directorcomment')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Director comment $directorcomment with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $directorcomment is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
              
              <h2 class="form-signin-heading">Update Director Comment</h2>	
			  
			  <label for="inputDirectorID" class="sr-only">Directorid</label>
              ...
              <br>
              <label for="inputDirectorcomment" class="sr-only">Comment</label>
              <input type="directorcomment" name="inputDirectorcomment" id="inputDirectorcomment" class="form-control" placeholder="new comment" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update Director Comment</button>
