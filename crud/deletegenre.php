        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="deletegenre.php">Delete Genre</a></li>
              <br>
              <!-- 8 language -->
		  ...
		  <br><br><br>
            ...
            <form class="form-signin"  method="POST" action = "deletegenre.php">
              <h2 class="form-signin-heading">Delete Genre</h2>
              <label for="inputGenreID" class="sr-only">Genre ID</label>
              <input type="number" name="inputGenreID" id="inputGenreID" class="form-control" placeholder="Movie ID" required autofocus>
              <br>
			  ...
			  // Delete movie
              if (!empty($_POST['inputGenreID'])){
                $id =($_POST['inputGenreID']);
                
                // prepare the query
                $stmt = "CALL DELETE_GENRE ('$id')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('Genre with ID $id is deleted at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> Genre is deleted.</div>";
                }
              }
              ob_end_flush();
              ?>
