          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-9 col-md-offset-3 main">
            <form class="form-signin" method="POST" action = "adduser.php">
              ...
              if (!empty($_POST['inputUsername']) && !empty($_POST['inputEmail']) && !empty($_POST['inputPassword']) && !empty($_POST['inputAge'])){
                ...
                // prepare the query
                $stmt = "CALL ADD_MEMBER ('$email','$pass','$username','$age', '2')";
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('A new user with username \"$username\" is added at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);

                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $username is added as an user.</div>";
                }
              }
              ob_end_flush();
              ?>
