        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="deleteactorcomment.php">Delete Actor Comment</a></li>
              <br>
			  <!-- 15 director comment -->
		  ...
		  <br><br><br>
            ...
            <form class="form-signin"  method="POST" action = "deleteactorcomment.php">
              <h2 class="form-signin-heading">Delete Actor Comment</h2>
              <label for="inputActorID" class="sr-only">Actor ID</label>
              <input type="number" name="inputActorID" id="inputActorID" class="form-control" placeholder="Actor ID" required autofocus>
              <br>
			  ...
			  // Delete movie
              if (!empty($_POST['inputActorID'])){
                $id =($_POST['inputActorID']);
                
                // prepare the query
                $stmt = "CALL DELETE_ACTOR_COMMENT ('$id','$memberID')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('Comment with ID $id is deleted at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> Comment is deleted.</div>";
                }
              }
              ob_end_flush();
              ?>
