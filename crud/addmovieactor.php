        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="addmovieactor.php">Add Movie Actor</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
			...
			<h2 class="form-signin-heading"></h2>	
              ...
              <br>
              <label for="inputGenrecontext" class="sr-only">Movietitle</label>
              <input type="genrecontext" name="inputGenrecontext" id="inputGenrecontext" class="form-control" placeholder="new genre context" required autofocus>
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update genre</button>
			
            <h2 class="form-signin-heading">List of Movie Actors</h2>
            ...
            // Prepare the query
            $stmt = "SELECT g.GENRE_ID,m.MOVIE_TITLE,g.GENRE_CONTEXT FROM GENRE g,MOVIE m WHERE m.MOVIE_ID=g.GENRE_ID ORDER BY GENRE_CONTEXT ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Movie</th> <th>Genre</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updategenre.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update movie
              if (!empty($_POST['inputMovieId']) && !empty($_POST['inputGenrecontext']) ){
                $id = ($_POST['inputMovieId']);
				$genrecontext = ($_POST['inputGenrecontext']);
                
                
                // prepare the query
                $stmt = "CALL UPDATE_GENRE ('$id','$genrecontext')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Movie genre $genrecontext with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id , $genrecontext is updated.</div>";
                }
              }
              ob_end_flush();
              ?>
