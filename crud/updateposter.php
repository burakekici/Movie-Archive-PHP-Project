        ...
        <div class="col-md-9">
          ...
          <div class="col-sm-3 col-md-2 sidebar">
            <ul class="nav nav-sidebar">
              ...
              <li class="active"><a href="updateposter.php">Update Poster</a></li>
          ...
          <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-3 main">
            ...
            <h2 class="form-signin-heading">List of Poster</h2>
            ...
            // Prepare the query
            $stmt = "SELECT p.POSTER_ID,m.MOVIE_TITLE,p.POSTER_PATH  FROM POSTER p,MOVIE m WHERE m.MOVIE_ID=p.POSTER_ID ORDER BY POSTER_ID ";
            ...
            if( !oci_execute($stid) ) {
              ...
              echo "<thead> <tr> <th>ID</th> <th>Movie</th> <th>Poster</th> </tr> </thead>";
              // Print the results
		  ...
		  <br><br><br>
            ...
            <form class="form-signin" method="POST" action = "updateposter.php">
              <?php
              require_once '../config.php';
              ob_start();
              
              // Update poster
              if (!empty($_POST['inputID']) && isset($_FILES["fileToUpload"]["name"])) {
							$id = ($_POST['inputID']);
                ...
                // prepare the query
                $stmt = "CALL UPDATE_POSTER ('$id','$imgpath')";
                $stid = oci_parse($conn, $stmt);
                ...
                if( !oci_execute($stid) ) {
                  ...
                  oci_close($conn);
					...
					$stmt = "CALL ADD_LOG('The Poster $movietitle with ID $id is updated at $currentTime on $currentDate.')";
					$stid = oci_parse($conn, $stmt);
					oci_execute($stid);
					
                  echo "<div class=\"alert alert-success\"><strong>Success!</strong> $id is updated.</div>";
                }
				header("Location:addposter.php");
                                    echo "<div class=\"alert alert-success\"><strong>Success!</strong></div>";
                                } else {
									echo "<div class=\"alert alert-danger\"><strong>Failed!</strong>Please select another picture.</div>";
								}
              ...
              <h2 class="form-signin-heading">Update Poster</h2>	
			  
			  <label for="inputID" class="sr-only">Movietitle</label>
              <input type="id" name="inputID" id="inputID" class="form-control" placeholder="Enter the ID you want to update" required autofocus>
              <br>
              <label for="inputMovietitle" class="sr-only">Movietitle</label>
              <input type="file" name="fileToUpload" id="fileToUpload">
              <br>
              <button class="btn btn-lg btn-primary btn-block" type="submit">Update poster</button>
